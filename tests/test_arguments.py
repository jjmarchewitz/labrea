...
def test_evaluatable_args():
    ...
    assert args.evaluate_options({"A": 1, "B": 2, "C": 3}) == {"A": 1, "B": 2, "C": 3}
...
def test_evaluatable_kwargs():
    ...
    # JAKE: why is this lowercase?
    assert kwargs.evaluate({"A": 1, "B": 2, "C": 3}) == {"a": 1, "b": 2, "c": 3}
    assert kwargs.evaluate_options({"A": 1, "B": 2, "C": 3}) == {"A": 1, "B": 2, "C": 3}
    ...
    assert (
        repr(kwargs) == "EvaluatableKwargs(a=Option('A'), b=Option('B'), c=Option('C'))"
    )
...
def test_evaluatable_arguments():
    ...
    assert args.evaluate_options({"A": 1, "B": 2}) == {"A": 1, "B": 2}
    args.validate({"A": 1, "B": 2})
    with pytest.raises(KeyNotFoundError):
        args.validate({"A": 1})
