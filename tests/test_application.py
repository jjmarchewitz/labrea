...
def test_application():
    ...
    assert app.evaluate_options({"A": 1, "B": 2}) == {"A": 1, "B": 2}
...
def test_lift():
    ...
    assert FunctionApplication.lift(good).evaluate_options({"A": 1, "B": 2}) == {
        "A": 1,
        "B": 2,
    }
    ...
    assert (
        FunctionApplication.lift(bad, a=Option("A"), b=Option("B")).evaluate(
            {"A": 1, "B": 2}
        )
        == 3
    )

    assert FunctionApplication.lift(bad, a=Option("A"), b=Option("B")).evaluate_options(
        {"A": 1, "B": 2}
    ) == {"A": 1, "B": 2}
    ...
    assert good_deco.evaluate_options({"A": 1, "B": 2}) == {"A": 1, "B": 2}
    ...
    assert good_deco_kwargs.evaluate_options({"A": 1, "B": 2}) == {"A": 1, "B": 2}
...
def test_lift_partial():
    ...
    assert PartialApplication.lift(some).evaluate_options({"B": 2}) == {"B": 2}

    assert PartialApplication.lift(none).evaluate({})(1, 2) == 3
    assert PartialApplication.lift(none).evaluate_options({}) == {}

    assert PartialApplication.lift(all).evaluate({"A": 1, "B": 2})() == 3
    assert PartialApplication.lift(all).evaluate_options({"A": 1, "B": 2}) == {
        "A": 1,
        "B": 2,
    }
    ...
    assert good_deco.evaluate_options({"B": 2}) == {"B": 2}

    @PartialApplication.lift(b=Option("B"))
    def good_deco_kwargs(a: float, b: float = Option("X")) -> float:
        return a + b

    assert good_deco_kwargs.evaluate_options({"B": 2}) == {"B": 2}
