...
from labrea.option import (AllOptions, Option,
                           UnrecognizedNamespaceMemberWarning,
                           WithDefaultOptions, WithOptions)
...
def test_basic():
    ...
    assert option.evaluate_options(options) == options
...
def test_multiple_provided():
    ...
    assert option.keys(options) == {'A'}
    assert option.explain(options) == {'A'}
    assert option.evaluate_options(options) == {'A': 42}
...
def test_missing():
    ...
    with pytest.raises(KeyNotFoundError) as excinfo:
        option.evaluate_options(options)
        assert excinfo.value.key == 'A'
...
def test_evaluatable_default():
    ...
    assert option.evaluate_options(options) == {'A': 42, 'V': 42}
...
def test_nested():
    ...
    assert option.evaluate_options(options) == {'A': {'V': 42}}
...
def test_repr():
    ...
    assert repr(Option('A', default='{V}.{C}')
                ) == f"Option('A', default={repr(Template('{V}.{C}'))})"
